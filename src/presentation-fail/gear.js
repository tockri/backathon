...
(function(global, $) {
  ...
  function Gear(m, z, options) {
    ...
    // 歯の形
    var t1 = {
      x: cr * Math.cos(a / 2) * 0.9,
      y: cr * Math.sin(a / 2) * 0.9
    };
    var t3 = {
      x: (d + 2*m) * Math.cos(a / 10),
      y: (d + 2*m) * Math.sin(a / 10)
    };

    function teeth(i) {
      var v2 = THREE.Vector2;
      var shape = new THREE.Shape([
        new v2(t1.x, t1.y), new v2(t3.x, t3.y),
        new v2(t3.x, -t3.y), new v2(t1.x, -t1.y)
      ]);
  ...
  var App = {
    ...
    // 画面初期化
    initialize: function() {
      $('body').append($('<button type="button">Start</button>')
      .css({
        position: 'absolute',
        right: 0,
        bottom: 0
